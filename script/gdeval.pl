...
sub topicDone {
  my ($printTopic, $runid, $topic, $pndcgTotal, $perrTotal, $pdcgTotal, $ptopics, $pseen, $pideal, 
      $isRiskSensitive, $riskAlpha, $baseNDCG, $baseERR, @gain) = @_;
  my($ndcg, $err, $dcg) = (0, 0, 0);
  if (exists($$pseen{$topic}) and defined($$pseen{$topic}) and $$pseen{$topic}) {
    $dcg = &dcg($K, @gain);
    $ndcg = $dcg/$$pideal{$topic};
    $err = &err ($K, @gain);
    ...
    $$pdcgTotal += $dcg;
    $$ptopics++;
    printf("$runid,$topic,%.5f,%.5f,%.5f\n",$ndcg,$err,$dcg) if ($printTopic);
    return ($ndcg,$err);
  }
}
...
sub processRun
{
  ...
  my $dcgTotal = 0;
  my $topics = 0;
  print "runid,topic,ndcg\@$K,err\@$K,dcg\@$K\n" if ($printTopics);
  my $topicCurrent = -1;
  my @gain = ();
  for (my $i = 0; $i <= $#run; $i++) {
	...
	my ($ndcg,$err) = &topicDone ($printTopics, $runid, $topicCurrent, \$ndcgTotal, \$errTotal, \$dcgTotal, \$topics, 
  ...
  printf "$runid,amean,%.5f,%.5f,%.5f\n",$ndcgAvg,$errAvg,$dcgAvg if ($printTopics);
